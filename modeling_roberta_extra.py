...
"""PyTorch RoBERTa assembled model."""
...
from transformers import PreTrainedModel, RobertaForMaskedLM
...
class RobertaAssembledModel(RobertaPreTrainedModel):
    """
...
class RobertaAssembledForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaAssembledModel(config, add_pooling_layer=False)
        self.lm_head = RobertaAssembledLMHead(config)
...
class RobertaAssembledLMHead(nn.Module):
...
class RobertaAssembledForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class RobertaAssembledForMultipleChoice(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.roberta = RobertaAssembledModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class RobertaAssembledForTokenClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class XLMRobertaAssembledForQuestionAnswering(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class RobertaForMaskedLMUpdated(RobertaForMaskedLM):
    _tied_weights_keys = ["lm_head.decoder.weight", "lm_head.decoder.bias"]
